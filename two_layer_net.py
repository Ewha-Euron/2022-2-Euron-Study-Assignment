...
Original file is located at
    https://colab.research.google.com/drive/1y2gbUTdleXeQaQWHIxZ4iqF1PdG-ESJy
"""
...
## Answer:
1, 2  
Sigmoid 활성화 함수는 값이 매우 크거나 작을 때 gradient가 죽는다.  
ReLU 함수는 양수에서는 gradient가 잘 작동하지만 음수에서는 ( x<0 일때) gradient가 죽는다.
...
$\color{blue}{\textit Your Explanation:}$ 1번 - 데이터가 많을수록 가중치가 좋아지고 학습의 정확도가 올라감  
3번- regularization strength를 높이면 특정 가중치가 너무 큰 영향을 미치지 않기 때문에 모델이 더욱 좋아진다
