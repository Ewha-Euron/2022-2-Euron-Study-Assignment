...
Original file is located at
    https://colab.research.google.com/drive/1mYcJWKFZzNJce5ssHNtjn6-p3bQcc-lD
"""
...
**Your Answer:** 
False. The saliency map differs from the gradient in that it is computed using the maximum of the absolute value of all input channels and is of the form (H, W), while the input image is of the form (3, H, W). This implies that for each pixel, (i) effectively only one channel will be used for gradient ascent for each of the 3 channels in the input image and, (ii) we lose the direction of the gradient since we're taking its absolute value. In other words, updates would occur only towards a single direction and the resulting pixels would approximate to very large numbers (towards +infinity) since updates to pixels would be all positive. To update the image using gradient ascent, we should use the gradient which is of the form (3, H, W), along with its direction/sign.
