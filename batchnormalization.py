...
Original file is located at
    https://colab.research.google.com/drive/1jFw-_1xZ8ncwVW7X00GMQWKdZE0p0dgW
"""
...
## Answer:
전체 정확도-batch normalization을 사용하면 전반적으로 정확도와 손실 결과 개선
robustness - 가중치의 작은 변화는 non-centered data에 대한 가중치를 변경하는 것과 비교하여 큰 출력 차이를 유발하지 않음
안정성 - batch normalization은 매우 크거나 매우 낮은 가중치 초기화에서도 정확도 결과 산출
...
배치 크기를 더 크게 사용하면 train accuracy가 높아짐. 배치 크기에 덜 의존하며 충분한 수의 에폭을 가짐
...
## Answer:
* 2 - feature 벡터에 몇 개의 값이 있으면 해당 형상의 평균과 분산에 근사하지 않으며 feature 벡터당 유사한 척도에 배치되지 않음
* 3 - 정규화 기간이 높으면 더 간단한 모델이 생성되고 손실도 증가하는 경향
"""
