...
## Answer: 
두번째 그림은 vanishing gradient 문제를 보여준다. baseline model은 이 문제에 민감하지만 batchnorm model은 덜 민감한 것을 알 수 있다.  
첫번째 그림은 두번째 그림과 유사하다. 첫번째 그림은 우리가 overfitting하고 있다는 것을 보여주며 batchnorm model이 기존보다 나은 결과를 보여준다.  
세번째 그림은 폭발하는 gradient 문제를 보여준다. 하지만 batchnorm에서는 이런 문제가 없다.
...
## Answer:
배치의 크기가 batch normalization에 직접적인 영향을 미친다는 것을 알 수 있다. 배치 크기가 클수록 성능이 우수하다.
...
## Answer:
2번 -> 정규화 결과 x^2/sum(x^2)  
3번 -> 정규화 결과 std*(x-mean)/std + 0 = x-mean
...
## Answer:
1. 잘 작동한다. 네트워크가 5개이므로 deep network이기 때문이다.
2. 잘 작동하지 않는다. feature가 작으면 노이즈가 커져 영향을 미친다.
3. 잘 작동하지 않는다. regularization term이 높으면 simple model을 학습한다.
"""
