...
Original file is located at
    https://colab.research.google.com/drive/1gidL6RVETxZ9XbjOTtwvPKCnTrFoT1jr
"""
...
## Answer: 
두번째 그림은 vanishing gradient 문제를 보여준다. baseline model은 이 문제에 민감하지만 batchnorm model은 덜 민감.
첫번째 그림은 두번째 그림과 유사. 첫번째 그림은 우리가 overfitting하고 있다는 것을 보여주며 batchnorm model이 기존보다 나은 결과를 보여준다.  
세번째 그림은 gradient가 폭발하는 문제를 보여준다. 하지만 batchnorm을 사용하면 그렇지 않는다.
...
## Answer:
According to the results, we can see that the batch size affects directly the performance of batch normalization (the smaller the batch size the worse). 
Even the baseline model outperforms the batchnorm model when using a very small batch size. 
This problem occurs because when we calculate the statistics of a batch, i.e., mean and variance, 
we try to find an approximation of the statistics of the entire dataset. 
Therefore with a small batch size, these statistics can be very noisy. On the other hand, with a large batch size we can obtain a better approximation.
...
## Answer:
[CORRECT] In the previous example, the network had five layers and it can be considered as a deep network. 
Thus, using layer normalization in deep networks works correctly.

[INCORRECT] Having a small dimension of features affects the performance of layer normalization. 
The problem is very similar to that of batch normalization with small batch size because in layer normalization we calculate the statistics according to the number of hidden units, which represent the features that the network is learning. Thus, the smaller the hidden size the noisier the statistics used in layer normalization.

[INCORRECT] Having a high regularization term affects the performance of layer normalization. 
In general, when the regularization term is very high, the model learns very simple functions (underfitting).
"""
