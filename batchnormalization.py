...
Original file is located at
    https://colab.research.google.com/drive/1fK56KODGlNT3SHLL9SEy25lyS42LkJqL
"""
...
FOLDERNAME = 'Euron/복습과제/assignment2'
...
## Answer:
- accuracy가 증가한다. 이유: Network's parameters are associated with each input and different scales of them can lead toward an awkward loss function topology which places more emphasis on certain parameter gradients.
...
## Answer:
batch size가 클 수록 training 데이터셋에 대해 높은 정확도를 보인다. 하지만 validation의 경우 batch size가 클수록 높은 정확도를 보이는 경향이 있지만, 일정 크기 이상의 에포크를 가지면 비슷해진다.
