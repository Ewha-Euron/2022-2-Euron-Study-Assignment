...
Original file is located at
    https://colab.research.google.com/drive/1z0h5BMjXErsepeT6gVPNdTVVvm0GWu3r
"""
...
FOLDERNAME = 'cs231n/assignments(2021)/assignment2/'
...
## Answer:
with batch normalization we can avoid the problem of vanishing and exploding gradients because it normalizes every affine layer (Wx+b) to have zero-centered data ( = 0) with unit variance ( = 1), i.e., properties of a standard normal distribution, thus avoiding very large/small values. Moreover, Batchnorm's inherent regularization properties allow us to reduce overfitting.
...
## Answer:
Applying Batchnorm only makes sense when we have decently large batch sizes. The performance of batch normalization is a function of the batch size. The smaller the batch size, worse the performance. Infact, for very small batch sizes (<= 5), the baseline model outperforms the model with Batchnorm.
...
3. True. Having a high regularization term affects the performance of layer normalization. In general, when the regularization term is very high, the model learns very simple functions (underfitting).
