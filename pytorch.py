...
Original file is located at
    https://colab.research.google.com/drive/1t72oTcV50DzyCZPhnQImMPgA4dYA0kEx
"""
...
FOLDERNAME = 'Euron/복습과제/assignments2'
...
def two_layer_fc(x, params):
    ...
    x = F.relu(x.mm(w1))
    x = x.mm(w2) # matrix multiplication
    return x
...
def three_layer_convnet(x, params):
    ...
    relu2 = F.relu(conv2)
    flat1 = flatten(relu2)
    print(f'relu2:{relu2.shape}, flat1:{flat1.shape}, fc_w:{fc_w.shape}')
    scores = flat1.mm(fc_w) + fc_b
...
conv_w1 = random_weight((channel_1, 3, 5, 5)) # conv weight [out_channel, in_channel, kH, kW]
...
class TwoLayerFC(nn.Module):
    def __init__(self, input_size, hidden_size, num_classes):
        ...
        nn.init.kaiming_normal_(self.fc1.weight) # 이거 왜 해주는거지??????????????
...
model = ThreeLayerConvNet(in_channel=3, channel_1=channel_1, channel_2 = channel_2, num_classes= 10) # in_channel, channel_1, channel_2, num_classes
optimizer = optim.SGD(model.parameters(), lr=learning_rate)
...
model = nn.Sequential(
    ...
    nn.ReLU(),
    nn.Conv2d(32, channel_2, kernel_size=3, padding=2),
    nn.ReLU(),
    Flatten(),
